...
export default function PrescriptionManager() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", color: "white" }}>
      ...
      <div style={{ width: "45%", padding: 20, borderRight: "1px solid #222", overflowY: "auto" }}>
        ...
        {medicines.map((m, index) => (
          <div key={index} style={{ marginBottom: 10 }}>
            <input className="input" placeholder="Name"
              value={m.name} onChange={(e) => updateMedicine(index, "name", e.target.value)} />

            <input className="input" placeholder="Dosage"
              value={m.dosage} onChange={(e) => updateMedicine(index, "dosage", e.target.value)} />

            <input className="input" placeholder="Duration"
              value={m.duration} onChange={(e) => updateMedicine(index, "duration", e.target.value)} />

            <input className="input" placeholder="Instructions"
              value={m.instructions} onChange={(e) => updateMedicine(index, "instructions", e.target.value)} />
          </div>
        ))}

        <button className="btn" onClick={addMedicineField}>+ Add Medicine</button>

        <h3>Upload PDF (optional)</h3>
        <input type="file" accept="application/pdf" onChange={(e) => setPdfFile(e.target.files[0])} />

        <button className="btn" style={{ marginTop: 20 }} onClick={handleSubmit}>
